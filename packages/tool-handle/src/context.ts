...
export interface ToolContext extends FormContext {
   ...
   */
  protocolHandlers: { readonly [name: string]: ProtocolHandler } | undefined;
...
export interface ToolContextOptions extends FormContextOptions {
   ...
   */
  protocolHandlers?:
    | readonly ProtocolHandler[]
    | { readonly [name: string]: ProtocolHandler }
    | undefined;
...
export function initToolContext(
  ...
  if (!("protocolHandlers" in context)) {
    context.protocolHandlers = undefined;
  }
  ...
  // Configure additional protocol handlers.
  if (options?.protocolHandlers !== undefined) {
    if (isArray(options.protocolHandlers)) {
      const protocolHandlers: Record<string, ProtocolHandler> = {
        ...context.protocolHandlers,
      };
      for (const protocolHandler of options.protocolHandlers) {
        protocolHandlers[protocolHandler.name] = protocolHandler;
      }
      context.protocolHandlers = protocolHandlers;
    } else if (context.protocolHandlers !== undefined) {
      context.protocolHandlers = {
        ...context.protocolHandlers,
        ...options.protocolHandlers,
      };
    } else {
      context.protocolHandlers = options.protocolHandlers;
    }
  }
